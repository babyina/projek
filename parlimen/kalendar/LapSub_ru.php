...
$jenis = "ru";
if(!empty($id)){
	$sql	= "SELECT * FROM kal_lapdwn_ru WHERE kal_lapdwn_ru_id='$id'";
	$sql	= "Select
				kal_lapdwn_ru.kal_lapdwn_ru_id,
				kal_lapdwn_ru.Kal_lapdwn_id,
				kal_lapdwn_ru.NamaYB,
				ahli_parlimen.id AS idAhliParlimen,
				kal_lapdwn_ru.Kawasan,
				kawasan.id AS idKawasan,
				kal_lapdwn_ru.Isu
				From
				kal_lapdwn_ru
				LEFT JOIN ahli_parlimen ON kal_lapdwn_ru.NamaYB = ahli_parlimen.nama
				LEFT JOIN kawasan ON kal_lapdwn_ru.Kawasan = kawasan.nama
			  WHERE kal_lapdwn_ru.kal_lapdwn_ru_id='$id'";
...
if($_POST['save']){
	...
	$isu		= $_POST['isu'];
	if(!empty($id))
		$sql		= "UPDATE kal_lapdwn_ru SET 
						NamaYB='".$namaYB."',
						Kawasan='".$kawasan."',
						Isu='".$isu."'
					   WHERE kal_lapdwn_ru_id='".$id."'";
	else
		$sql		= "INSERT kal_lapdwn_ru SET 
...
<script language="javascript">
	function redirect(){
		...
		var url = 'LapSub_ru.php';
		var id = frm_ru.id.value;  //parent id
		var pid = frm_ru.parent_id.value;  //parent id
		var kawasan = frm_ru.kawasan.value;  //kawasan
...
<head>
<title>Maklumat Rang Undang-undang</title>
...
<form method="post" enctype="multipart/form-data" name="frm_ru">
	<fieldset>
	<legend>Maklumat Rang Undang-undang</legend>
	<table width="100%">
		<tr>
