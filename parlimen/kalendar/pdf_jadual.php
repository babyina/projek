...
$query = "select Tarikh, Sesi, PegawaiBtugas, Dewan, Agensi from kal_pegawaitugas where Kal_mesyuarat_id ='$mesyuarat_id' order by Tarikh, Sesi";
...
while($row = mysql_fetch_array($sql)){
	...
	if(!empty($pegawai)){
			$telOPgw	= lookup($conn, "pengguna", "telefon", "nama='$pegawai'");
			$telHpPgw	= lookup($conn, "pengguna", "handphone", "nama='$pegawai'");
			$emailPgw	= lookup($conn, "pengguna", "emel", "nama='$pegawai'");	
		}
	...
	if (!$dplcateDewan){
	$pdf->SetFont('Arial','B',12);
	if ($currDewan == 'negara'){$pdf->AddPage();
	}
	...
	$pdf->Row(array($print_hari,$print_date,$pegawai.$details,''));
