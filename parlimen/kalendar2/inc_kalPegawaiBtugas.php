	  ...
	  while($rowPgw=mysql_fetch_array($rsPgw)){
		...
		<?php }
			...
			<tr>
				...
				if($isCalHek && ($_POST['KalEdit'])){
					if(!$duplicate){ ?>
						<img src="images/bt_edit.png" alt="Catatan Cuti/Tiada Bersidang" style="cursor:pointer" onClick="NewWindow('KalSub_hari.php?id=<?php echo $id;?>&tkh=<?php echo $rowPgw['Tarikh'];?>&hari=<?php echo $hari;?>','editPegawai',650,400,true)">
						<img src="images/delete.gif" alt="Hapus hari ini">
				<?php 
					}
				} ?>
				</td>
				<td class=" <?php echo $alt; ?>"><?php echo (!$duplicate)?$currDate:""?></td>
				<td class=" <?php echo $alt; ?>"><?php echo $rowPgw['Sesi']?></td>
