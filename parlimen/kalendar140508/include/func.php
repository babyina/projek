...
function setPetugas($agensi,$id){
	...
	Setelah itu mark kan dia pernah bertugas pada field TugasMesyuarat
...
<? //----------------azila add utk format tarikh di page tambah.php
...
function dateToMysql2($datestr){
		list($day,$month,$year)=split("/",$datestr);
		return $year."-".$month."-".$day;
	}

function DateFromMysql($datestr)
	{
		if ($datestr==NULL or $datestr==0000-00-00){
			return "-";
		}
		else{
		list($year,$month,$day)=split("-",$datestr);
		if(strstr($month,"0")){
			list($sampah,$month)=explode("0",$month);
		}
	
		if(strstr($day,"0")){
			list($sampah,$day)=explode("0",$day);
		}
...
function DateFromMysql2($datestr)
	{
		if ($datestr==NULL or $datestr==0000-00-00){
			return "-";
		}
