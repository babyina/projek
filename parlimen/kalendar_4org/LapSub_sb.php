...
if(!empty($id)){
	...
	$sql	= "Select
				kal_lapdwn_sb.Kal_lapdwn_sb_id,
				kal_lapdwn_sb.Kal_lapdwn_id,
				kal_lapdwn_sb.NamaYB,
				ahli_parlimen.id AS idAhliParlimen,
				kal_lapdwn_sb.Kawasan,
				kawasan.id AS idKawasan,
				kal_lapdwn_sb.Soalan
				From
				kal_lapdwn_sb
				LEFT JOIN ahli_parlimen ON kal_lapdwn_sb.NamaYB = ahli_parlimen.nama
				LEFT JOIN kawasan ON kal_lapdwn_sb.Kawasan = kawasan.nama
			  WHERE kal_lapdwn_sb.Kal_lapdwn_sb_id='$id'";
...
if($_POST['save']){
	...
	$soalan		= $_POST['soalan'];
	if(!empty($id))
		$sql		= "UPDATE kal_lapdwn_sb SET 
						NamaYB='".$namaYB."',
						Kawasan='".$kawasan."',
						Soalan='".$soalan."'
					   WHERE Kal_lapdwn_sb_id='".$id."'";
	else
...
<script language="javascript">
	function redirect(){
		...
		var url = 'LapSub_sb.php';
		var id = frm_sb.id.value;  //parent id
		var pid = frm_sb.parent_id.value;  //parent id
		var kawasan = frm_sb.kawasan.value;  //kawasan
...
<form method="post" enctype="multipart/form-data" name="frm_sb">
	<fieldset>
	...
	<table width="100%">
		...
		<tr>
			...
			<td>:</td>
			<td>
			<?php createRTF($sBasePath, 'soalan', $row['Soalan']);?>
			&nbsp;</td>
		</tr>
		<tr>
		  <td>Lampiran</td>
