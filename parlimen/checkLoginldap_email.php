...
if($kodkategori >= 1 and $kodkategori <= 5 ){//1
	...
	if($user_!="")//if password and username not empty
			{//5
				...
				if($ldapconn2)
				{//6
			  $login_ldap = ldap_bind($ldapconn2,$user_,$idpwd) or die("<script>window.alert('No KP dan Katalaluan tidak sepadan');location.href='login.php?action=details&id=$idpost';</script>");
			//echo "test"; 
					...
					//$abc=107;
		               ...
		               $_SESSION['_login_id'] = mysql_insert_id();
						...
						echo("<script>location.href='parlimen/index.php?action=details&id=$idpost';</script>");
						/*echo("<script>location.href='mainmenu.php';</script>");*/
						}
						else
						{
						echo ("<script>window.alert('Pengguna Belum Berdaftar');location.href='login.php?action=details&id=$idpost';</script>");
						}
				//echo $Login. "<BR>";
			...
			echo ("<script>window.alert('No KP dan Katalaluan tidak sepadan');location.href='login.php?action=details&id=$idpost';</script>");
...
$_SESSION['jns_kategori']=1;
if($Login=="" && $Pswd=="")
{//12header("location:login.php?valid=false&action=details&id=".$_POST['id']);

echo ("<script>window.alert('Sila Masukkan NoKP Dan Katalaluan Anda!!');location.href='login.php?action=details&id=$idpost';</script>");
}//12
...
if($Login=="" && $Pswd=="")
{
echo ("<script>window.alert('Sila Masukkan No KP  Dan Katalaluan Anda!!');location.href='login.php?action=details&id=$idpost';</script>");
}
if($Login=="" && $Pswd!=""){
echo ("<script>window.alert('No KP dan Katalaluan tidak sepadan');location.href='login.php?action=details&id=$idpost';</script>");
}
...
{
	//echo "masukkan pwd";	
	echo ("<script>window.alert('Sila Pilih Kategori!!');location.href='login.php?action=details&id=$idpost';</script>");
}
