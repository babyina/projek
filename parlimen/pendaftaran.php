...
if(isset($_POST['daftar'])){
	...
	if(mysql_query($sql)){
		...
		$url = "http://".$_SERVER['HTTP_HOST']."/ssjp/"."tadbir/daftar/index.php?action=details&id=" . $idUser;

		$subject = "Pendaftaran Pengguna Sistem Pengesanan Soal Jawab Parlimen ";
		$message .= "Tuan/Puan,\n\n";
		$message .= "Saya " . $nama . " (" . $jawatan . ") " . $agensi_nama . " ingin memohon untuk mendaftar ";
		$message .= "sebagai pengguna Sistem Pengesanan Soal Jawab Parlimen \n\n";
		...
		// email HEK /parlimen/KSU 
		$roles_array = array();
		$query = mysql_query("SELECT roles, emel FROM pengguna WHERE roles LIKE '%3%' AND roles LIKE '%9%'");
		...
		if($error > 0){
			...
			echo $err_msg;//= "Maaf. Emel permohonan anda gagal dihantar";
