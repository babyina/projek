        ...
        switch($argv[1]) {
            ...
            if (ereg('^http[s]?://',$argv[1])) {
                ...
                die("Usage: php -f spider.php [option]\n"
                    ...
                    ."\tall (default)\n"
                    ."\tforceall\n"
                    ."\thttp://something or http://something\n"
...
if ($from_shell_flag == 2 && isset($url) && $url && $url != 'http://' && $url != 'http://' && (!$respider_mode || $respider_mode == 'site')) {
    extract(phpdigGetSiteFromUrl($id_connect,trim($url),$linksper,$linksper_flag,$limit,$limit_flag,$usetable));
}
elseif (isset($url) && !empty($url) && $url != 'http://' && $url != 'http://' && (!$respider_mode || $respider_mode == 'site')) {
    $urlsBox = preg_split("/[\r\n]+/",$url);
  ...
  while ($site_datas = array_pop($list_sites)) {
    ...
    if (is_array($verify)) {
        ...
        if ($site_datas['user'] && $site_datas['pass']) {
            if (substr($site_datas['site_url'],0,5) == "http") {
