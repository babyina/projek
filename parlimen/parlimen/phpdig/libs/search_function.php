...
if ((is_array($final_result)) && (count($final_result) > 0)) {
    ...
    for ($n = 1; $n <= $n_end; $n++) {
		...
		$url = $home_site_url."/'parlimen/parlimen/lampiran/';".$content['file'];
   ...
   $Carian = $_GET['Carian'];
   $url_bar = SEARCH_PAGE."?action=search&Carian=$Carian&amp;template_demo=$template_demo&amp;browse=1&amp;query_string=".urlencode($my_query_string_link)."$refine_url&amp;limite=$limite&amp;option=$option&amp;lim_start=";
