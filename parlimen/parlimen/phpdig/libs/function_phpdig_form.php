...
$Carian = $_GET['Carian'];
function phpdigMakeForm($query_string="",$option="start",$limite=10,$result_page="",$site="",$path="",$mode='classic',$template_demo="",$num_tot=0,$refine=0)
{
  ...
  if (($num_tot == 0) || (empty($site) && empty($path))) {
    ...
    if (isset($site) && is_numeric($site) && ($site > 0)) {
       ...
       if (DROPDOWN_URLS) {
		...
		$Carian = $_GET['Carian'];
            $result['form_head'] = "<form action='?action=search&Carian=$Carian' method='post'>
            ...
            $result['form_dropdown'] ="<select name='path' style='display:none;'>";
...
$Carian = $_GET['Carian'];
      ...
      <input type='hidden' name='template_demo' value='$template_demo'/>
      <input type='hidden' name='result_page' value='$result_page'/>";
      $result['form_dropdown'] =" <select  name='site' style='display:none;'>";
      //$result['form_dropdown'] .= "<option value=''>".phpdigMsg('searchall')."</option>";
	 // $result['form_dropdown'] = "http://localhost/spkweb/parlimen/parlimen/lampiran/";
	 // $dd_data['site_url']= "http://localhost/spkweb/";
	 // $dd_data['path'] ="parlimen/parlimen/lampiran/";
      while ($dd_data = mysql_fetch_array($dd_query)) {
...
$result['form_field'] = "<input type='hidden' class='phpdiginputtext' size='".SEARCH_BOX_SIZE."' maxlength='".SEARCH_BOX_MAXLENGTH."' name='query_string' value='".htmlspecialchars(stripslashes($Carian),ENT_QUOTES)."'/>";
...
//$result['form_button'] = "<input type='submit' name='search' value='Senarai Lampiran'/>"; kes senarai lapiran  ubah pada 25/1/2010   
