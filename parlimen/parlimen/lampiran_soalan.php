...
//===================================================
function insert_db($parlimen_id, $file_name)
{
	//if(!empty($content))
	//{					
		#gantikan dgn table parlimen in future
		$result = mysql_query("insert into soalan_lampiran(parlimen_id, nama_fail) values('$parlimen_id', '$file_name')") or die (mysql_error());
		if($result)
			return true;
		else
			return false;
    ...
    switch($file['error'])
 	{
  		case 0:
			...
			/*
				...
				{					
					if(insert_db($parlimen_id, $file['name']) == true)
