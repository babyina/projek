...
session_start();
	...
	if($view_name == "bydate"){
		...
		ORDER BY parlimen.tkh_bentang_jawapan $sortTkh,
		nama_yb $sortWakil 
		LIMIT $offset,$pgRow";
	...
	if($view_name == "byJwpnAkhirSoal"){
		...
		"<a href='index.php?action=list&view=byJwpnAkhirSoal&sortTarikh=asc&sortWakil=$sortWakil'><img src=\"../images/asc.gif\" border=\"0\"></a>Tarikh<img src=\"../images/altdesc.gif\">";
	...
	if($view_name == "byyb"){
		...
		AND  (parlimen.agensi LIKE '%+$Pegawai_Agensi' OR parlimen.agensi LIKE '$Pegawai_Agensi+%' OR parlimen.agensi LIKE '%+$Pegawai_Agensi+%' OR parlimen.agensi LIKE '$Pegawai_Agensi')
	...
	if($view_name == "byno"){
		...
		$qry = "SELECT parlimen.id, DATE_FORMAT(tkh_bentang_jawapan,'%d/%m/%Y') AS Tarikh,ahli_parlimen.nama AS nama_yb, bentuk_soalan,no_soalan,perkara
				FROM parlimen,ahli_parlimen
				WHERE parlimen.ahli_dewan_id = ahli_parlimen.id $sortAhli
				ORDER BY no_soalan ASC, Tarikh desc LIMIT $offset,$pgRow";
		//echo $qry;
...
if($view_name == "byagensi"){
		?> <div class="tajuk">Soal Jawab Parlimen - Mengikut Agensi</div>
		<br />
		<br /> <?php
	...
	if($view_name == "imbasan"){
		...
		$qry = "SELECT parlimen.id,parlimen.perkara, DATE_FORMAT(tkh_mula_bersidang,'%d/%m/%Y') AS Tarikh,parlimen_lampiran.nama_fail
				...
				ORDER BY tkh_mula_bersidang DESC LIMIT $offset,$pgRow";
	...
	if($view_name =='perhatianSoal' || $view_name ==''){
		...
		if($isPegawai){
			...
			AND (parlimen.agensi LIKE '%+$Pegawai_Agensi' OR parlimen.agensi LIKE '$Pegawai_Agensi+%' OR parlimen.agensi LIKE '%+$Pegawai_Agensi+%' OR parlimen.agensi LIKE '$Pegawai_Agensi') GROUP BY parlimen.id ORDER BY tkh_bentang_jawapan desc";
			}
		elseif($isHEK)
			$qry = "SELECT parlimen.id,parlimen.perkara,DATE_FORMAT(parlimen.tkh_bentang_jawapan,'%d/%m/%Y') AS Tarikh,parlimen. bentuk_soalan AS BentukSoalan,ahli_parlimen.nama AS AhliDewan,
			parlimen.ahli_dewan_id, parlimen.no_soalan AS NoSoalan,parlimen.status AS status
			FROM parlimen, ahli_parlimen 
			WHERE parlimen.ahli_dewan_id=ahli_parlimen.id AND (parlimen.status = 1 OR parlimen.status = 3 OR parlimen.status=5 OR parlimen.status = 7 OR parlimen.status=8 OR parlimen.status=23) ORDER BY tkh_bentang_jawapan desc";
	
		elseif($isBoth)
			$qry = "SELECT parlimen.id,parlimen.perkara,DATE_FORMAT(parlimen.tkh_bentang_jawapan,'%d/%m/%Y') AS Tarikh,parlimen. bentuk_soalan AS BentukSoalan,ahli_parlimen.nama AS AhliDewan,
			parlimen.ahli_dewan_id, parlimen.no_soalan AS NoSoalan,parlimen.status, parlimen.penyemak  
			FROM parlimen, ahli_parlimen
			WHERE parlimen.ahli_dewan_id=ahli_parlimen.id AND (parlimen.status = 6 OR parlimen.status = 4) AND (parlimen.penyemak LIKE '$Jawatan') ORDER BY tkh_bentang_jawapan desc";
	
		elseif($isPengesahan)	
			$qry = "SELECT parlimen.id,parlimen.perkara,DATE_FORMAT(parlimen.tkh_bentang_jawapan,'%d/%m/%Y') AS Tarikh,parlimen. bentuk_soalan AS BentukSoalan,ahli_parlimen.nama AS AhliDewan,
			parlimen.ahli_dewan_id, parlimen.no_soalan AS NoSoalan,parlimen.status AS status, parlimen.penyemak  
			FROM parlimen, ahli_parlimen
			WHERE parlimen.ahli_dewan_id=ahli_parlimen.id AND (parlimen.status = 6) AND (parlimen.penyemak LIKE '$Jawatan') ORDER BY tkh_bentang_jawapan desc";
	
		elseif($isPengurusan)
			$qry = "SELECT parlimen.id,parlimen.perkara,DATE_FORMAT(parlimen.tkh_bentang_jawapan,'%d/%m/%Y') AS Tarikh,parlimen. bentuk_soalan AS BentukSoalan,ahli_parlimen.nama AS AhliDewan,
			parlimen.ahli_dewan_id, parlimen.no_soalan AS NoSoalan,parlimen.status AS status, parlimen.penyemak 
			FROM parlimen, ahli_parlimen 
			WHERE parlimen.ahli_dewan_id=ahli_parlimen.id AND (parlimen.status = 4)  AND (parlimen.penyemak LIKE '$Jawatan') ORDER BY tkh_bentang_jawapan desc";
		...
		if($rows == 0){
		echo "Tiada data untuk tindakan buat masa ini. Sila KLIK menu SENARAI DATA untuk membuat semakan.";
		}
	}
