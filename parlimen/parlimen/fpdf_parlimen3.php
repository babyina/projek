...
$pdf->Ln($fontSize*1.5);
$pdf->MultiCell(0,0,"SOALAN NO: ".$no_soalan,0,'R');
$pdf->Ln($fontSize*1.5);
$pdf->Ln($fontSize*1.5);
$fontSize = 15;
...
$pdf->Ln($fontSize*1.5);
$pdf->MultiCell(0,0,strtoupper(($rows['sesi_dewan']==1)?"Dewan Rakyat":"Dewan Negara").", MALAYSIA",0,'C');
$pdf->Ln($fontSize*0.5);
$pdf->MultiCell(0,0,"",1,'C');
$pdf->Ln($fontSize*2.5);
$y = $pdf->GetY();
$y += $fontSize*2;
$pdf->SetFont('Arial','B',$fontSize);
...
$pdf->SetX($x2 + 10); $pdf->Cell(0,$fontSize,strtoupper($rows['bentuk_soalan']),0,1);
$pdf->Ln($fontSize*0.5);
$pdf->Ln($fontSize*0.5);

$YBid	= $rows['ahli_dewan_id'];
$sqlYB	= "SELECT nama,pangkat FROM ahli_parlimen WHERE id='$YBid'";
...
$pdf->Cell(0,$fontSize,":");
$pdf->SetX($x2 + 10);
$pdf->MultiCell(0,$fontSize,$gelarYB.' '.$nama,0,'L');
$pdf->SetX($x2 + 10);
$pdf->Cell(0,$fontSize,$nama,0,1);
$pdf->Ln($fontSize*0.5);

$hari = findHari($row['tkh_bentang_jawapan']);
$pdf->Cell(0,$fontSize,"TARIKH");
$pdf->SetX($x2); $pdf->Cell(0,$fontSize,":");
//$pdf->SetX($x2 + 10); $pdf->Cell(0,$fontSize,formatDate($rows['tkh_bentang_jawapan'])." ($hari)",0,1);
$pdf->SetX($x2 + 10); $pdf->Cell(0,$fontSize,formatDate($rows['tkh_bentang_jawapan']),0,1);
...
//Maklumat Tambahan pada muka baru
$pdf->AddPage();
$pdf->SetFont('Arial','B',$fontSize);
$pdf->Ln($fontSize*2);
$pdf->Cell(0,$fontSize*1.5, "MAKLUMAT TAMBAHAN :",0,1);	
...
$pdf->Ln($fontSize);$pdf->Cell(0,$fontSize,"TARIKH");$pdf->SetX($x2);$pdf->Cell(0,$fontSize,":");$pdf->SetX($x2+10);$pdf->Cell(0,$fontSize,DisplayDate($created_on),0,1);
