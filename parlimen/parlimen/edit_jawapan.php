...
if(checkACL($_SESSION['userid'],2,$_SESSION['agensi_id'],$conn) == false){
	...
	$jawapan=preg_replace('/<a name="OLE_LINK([0-9])">(.*?)<\/a>/i', '\\2',$jawapan); 
	$tambahan = $row3['tambahan'];
	$tambahan =preg_replace('/<a name="OLE_LINK([0-9])">(.*?)<\/a>/i', '\\2',$tambahan);
...
<div class="subheader1"><center><img src="../images/dot.gif"/> SOAL JAWAB PARLIMEN <img src="../images/dot.gif"/></center></div><br/>
	...
	function getTKSP_KSP($def,$conn,$kategori){ //checkbox
	   $qry = "SELECT * FROM pengguna WHERE jawatan LIKE '%TKSP%' and jawatan not LIKE 'pa%'";
		...
		while($row = mysql_fetch_array($result)){
			...
			$checked = "";
					...
					//if($id=='TKSP (S&K)')
						...
						$id=$id."/KSP";
						//}
						$td = "<input readonly type=\"text\" name=\"tksp2\" value=\"$id\">";
						echo $td;
...
var valid, el, els = document.getElementsByName("Fail[]"); 

var inputs = document.getElementsByTagName("input");  
var valid_fail = false;
var nama_fail;
var nama_find;
var sms="";
var disediakan;
var test;
var benda_dicari_decode;
var benda_dicari;
...
{  
  ...
  //benda_dicari_decode=inputs[i].value;
   benda_dicari=inputs[i].value;
   benda_dicari_nama=inputs[i].value;
    var fakePathString = "fakepath";

		
            //if (benda_dicari.indexOf(fakePathString) != 0)
			if(benda_dicari.search(fakePathString)!= -1)
            {
               
                benda_dicari = benda_dicari.substring(12);
				benda_dicari_nama=benda_dicari_nama.substring(12);
            }
 //alert("atas"+benda_dicari)
   //benda_dicari= benda_dicari.substring(12);
   //alert("cod"+benda_dicari)
   benda_dicari=md5(base64_encode(benda_dicari))
    //alert("kod"+benda_dicari)
   //benda_dicari_decode=base64_decode(benda_dicari)
   //test=base64_decode(disediakan_2)
    
	//alert("no"+i+benda_dicari); 
	for(var n=0, len=els.length; n<len; n++){
	    el = els[n];
		
		 //disediakan="/"+el.value+" /i";
		  disediakan=el.value; 
		  disediakan_2=el.value; 
			
		if(benda_dicari.toLowerCase().search(disediakan.toLowerCase())!= -1){
			//alert("jumpa");
			
			valid_fail=true
			 //disediakan=base64_decode(disediakan)
			nama_fail=disediakan.toLowerCase();  
			nama_find=base64_decode(disediakan_2)
			sms+="Fail Lampiran "+benda_dicari_nama+" Telah Wujud Sila Tukar Fail Lain"+"\n";
...
*/
var EditorInstance = FCKeditorAPI.GetInstance('Jawapan');    //location_info is name of text area.
var Jawapan = EditorInstance.GetXHTML(true);
if (Jawapan=="") {
themessage = themessage + " - Jawapan";
}
...
if (document.edit_jawapan.jns_soalan.value=="") {
themessage = themessage + " - Bentuk Soalan";
}


if (themessage != "Sila Isikan Maklumat Berikut: ") {
alert(themessage);
//alert(document.edit_jawapan.jns_soalan.value);
return false;
} else {
//alert(document.edit_jawapan.jns_soalan.value);
edit_jawapan.submit();
...
var td_bentuk_soalan= document.getElementById("jns_soalan");
	if(val=='s1')
	{
	document.getElementById("operasi").style.display='';
	document.getElementById("dasar").style.display='none';
	td_bentuk_soalan.value = 's1';
	}
	if(val=='s2')
	{
	document.getElementById("operasi").style.display='none';
	document.getElementById("dasar").style.display='';
	td_bentuk_soalan.value = 's2';
	}
}

</script>
<form name="edit_jawapan" enctype="multipart/form-data" method="post" onsubmit="if(toValidate) return verify();if(simpan) return verify_simpan(this);">
	...
	<div class="sub">
		...
		<?php //echo $jawapan_id."<br>" ?>
			<tr>
			  ...
			  <td>&nbsp;</td>
			  <td>
              <input type="hidden" name="jns_soalan" id="jns_soalan"/><input name="Perkara" type="hidden" id="Perkara" value="<?php echo $_POST['Perkara'];?>" />
			...
			<tr><td width=198>Dihantar Kepada</td><td width=3>:</td>
			...
			while($row3 = mysql_fetch_array($res)){
				...
				echo "<input type=\"submit\" value = \"Hapus\" name=\"hapuslampiran\" class=\"button\" onClick=\"toValidate=false;simpan=false\"/><br>";
		...
		get_bentukSoalan($conn,$cat,$parlimen_id);
           ...
           <tr id="operasi" style="display:none;">
          <td width="198">Peringkat kelulusan:</td>
          <td>:</td>
          <td><?php  //echo  "jaw sta".$status;
			$key = $keyword[20];
			//getSalinan($salinan,$key,$conn,$sub_jawatan);
 			  getTKSP($penyemaktksp,$conn,$sub_jawatan);
           ...
           <tr id="dasar" style="display:none;">
          <td width="198">Peringkat kelulusan:</td>
          <td>:</td>
          <td><?php  //echo  "jaw sta".$status;
			$key = $keyword[20];
			//getSalinan($salinan,$key,$conn,$sub_jawatan);
 			  getTKSP_KSP($penyemaktksp,$conn,$sub_jawatan);
