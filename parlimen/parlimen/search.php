...
while(list( ,$value) = each($findme2)){
	...
	if(empty($Carian))
	{
	$Carian = ($_POST['Date'])? $_POST['Date']:$_GET['Date'];
	//$Carian = $_POST['Date'];
	}

	$datesearch=terbalikkan_tarikh($Carian); 
		//echo $Carian .	$datesearch;
...
$qry = "SELECT 
		...
		parlimen.tkh_mula_bersidang LIKE '$datesearch' OR 
				
		parlimen.soalan LIKE '%$Carian%' OR 
		
		parlimen.korperat_jawapan LIKE '%$Carian%') AND
		...
		parlimen.korperat_jawapan LIKE '%$Carian%'
		 order by parlimen.tkh_mula_bersidang asc";}
