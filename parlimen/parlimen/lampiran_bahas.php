...
function checkLampiran($bahas_id,$conn)
{
	$qry3 = "SELECT lampiran from sesi_bahas WHERE sesi_bahas.id = '$bahas_id' LIMIT 1";
...
//===================================================
function insert_db($jawapan_id, $bahas_id, $file_name)
{
	//if(!empty($content))
	//{	//echo $jawapan_id;	
		#gantikan dgn table parlimen in future
		$result = mysql_query("insert into bahas_lampiran(jawapan_id, bahas_id, nama_fail) values('$jawapan_id', '$bahas_id', '$file_name')") or die (mysql_error());
		if($result)
			return true;
		else
			return false;
  		...
  		case 0:
		   ...
		   if($file['name'] != NULL)
			...
			/*
				...
				{					
					if(insert_db($jawapan_id, $bahas_id, $file['name']) == true)
