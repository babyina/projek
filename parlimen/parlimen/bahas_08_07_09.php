...
elseif($_GET['action']=='listbahas' && $_GET['view']=='bytarikh'){
	...
	$qry = "select id,DATE_FORMAT(tkh_mula,'%d/%m/%Y') AS Tarikh,tajuk,menteri from sesi_bahas order by tkh_mula DESC, tajuk ";
...
elseif($_GET['action']=='listbahas' && $_GET['view']=='bysesi'){
	...
	$view->limit = "LIMIT $offset,$pgRow";
	$qry_all = "SELECT COUNT(*) AS total FROM sesi_bahas";
	$qry = "SELECT id,DATE_FORMAT(tkh_mula,'%d/%m/%Y') AS TarikhMula,tajuk,menteri FROM sesi_bahas ORDER BY tajuk, tkh_mula desc ";
