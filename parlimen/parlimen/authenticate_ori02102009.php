...
//	$myAccess = new Access($ldapServer,$ldapPort,$ldapBase);	  
	...
	//if(isset($userid) && isset($password)&&($validpass)) ini
	//{ ini
		...
		//$userid = $_POST['userid'];
		//$password = $_POST['password'];
		$qry = "SELECT pengguna.nama as nama, pengguna.jawatan, pengguna.agensi_id, pengguna.telefon FROM pengguna WHERE pengguna.id = '$userid' AND pengguna.password=MD5('$password')";
		...
		if($_SESSION['agensi_id']<>0)
		{
			...
			$qry3 = "SELECT kod, butiran FROM konfigurasi WHERE butiran ='$jwtn_temp'";
			$result3 = mysql_query($qry3,$conn) or die(mysql_error());

			if(mysql_num_rows($result3)>0)
			{
				$row3 = mysql_fetch_array($result3);
				//$_SESSION['jawatan'] = $row3['kod'];
			}
		}
	...
	$action = $_POST['id'];
		...
		if(isset($_POST['action'])){
			...
			*/			
			...
			elseif($_POST['action']=='detailsjm') {
				header("location:kabinet/index.php?action=detailsjm&id=".$_POST['id']);
			}
			else{
				header("location:mainmenu.php"); 
			}
