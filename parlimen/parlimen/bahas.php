...
$act = $_GET['action'];
$view_name = $_GET['view'];
if ($meet != ""){$query_meet = "AND mesyuarat = '$meet'";} else{ $query_meet == "";}

if(($_GET['view'] == 'bytarikh') || ($_GET['view'] == 'bysesi') || ($_GET['view'] == 'bystatus2') || ($_GET['view'] == 'byJwpnAkhirBalas')){ ?>
	<table width=100% border=0 cellspacing=1 style="border-collapse:collapse">
	<tr valign="top">
		    <td width="7%">Mesyuarat :</td>
...
elseif($_GET['action']=='listbahas' && $_GET['view']=='bytarikh'){
	...
	$qry = "select id,DATE_FORMAT(tkh_mula,'%d/%m/%Y') AS Tarikh,tajuk,menteri from sesi_bahas WHERE YEAR(tkh_mula) = '$tahun' $query_meet order by tkh_mula DESC, tajuk ";
	...
	<br><?php
	$view->out();
	$ref="<a href=\"".$_SERVER['PHP_SELF']."?&action=listbahas&view=bytarikh&meet=$meet&page=";
...
elseif($_GET['action']=='listbahas' && $_GET['view']=='byJwpnAkhirBahas'){
	...
	<br><?php
	$view->out();
	$ref="<a href=\"".$_SERVER['PHP_SELF']."?&action=listbahas&view=byJwpnAkhirBahas&meet=$meet&page=";
	$view->Paging($pgRow,$ref,isset($_GET['page'])?$_GET['page']:1);	
}//listing
...
elseif($_GET['action']=='listbahas' && $_GET['view']=='bysesi'){
	...
	$view->limit = "LIMIT $offset,$pgRow";
	$qry_all = "SELECT COUNT(*) AS total FROM sesi_bahas";
	$qry = "SELECT id,DATE_FORMAT(tkh_mula,'%d/%m/%Y') AS TarikhMula,tajuk,menteri FROM sesi_bahas WHERE YEAR(tkh_mula) = '$tahun' $query_meet ORDER BY tajuk, tkh_mula desc ";
	...
	<br><?php
	$view->out();
	$ref="<a href=\"".$_SERVER['PHP_SELF']."?&action=listbahas&view=bysesi&meet=$meet&page=";
	$view->Paging($pgRow,$ref,isset($_GET['page'])?$_GET['page']:1);	
}//listing
...
elseif($_GET['action']=='listbahas' && $_GET['view']=='bystatus2'){
	...
	$view->limit = "LIMIT $offset,$pgRow";
	$qry_all = "SELECT COUNT(*) AS total FROM sesi_bahas";
	$qry = "SELECT id,tkh_mula AS TarikhMula,tajuk,menteri,status FROM sesi_bahas WHERE YEAR(tkh_mula) = '$tahun' $query_meet ORDER BY TarikhMula DESC ";
	...
	<br><?php
	$view->OutCat();
	$ref="<a href=\"".$_SERVER['PHP_SELF']."?&action=listbahas&view=bystatus2&meet=$meet&page=";
	$view->Paging($pgRow,$ref,isset($_GET['page'])?$_GET['page']:1);	
}//listing
?>
</body>