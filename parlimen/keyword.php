<?php
	$query_perkara = "SELECT butiran FROM konfigurasi WHERE kategori='Perkara' ORDER BY butiran ASC";
	$query_parlimen = "select butiran from konfigurasi where kategori='Sesi Parlimen' order by CAST(kod AS unsigned) ASC";
	$query_penggal = "select distinct(butiran) from konfigurasi where kategori='Penggal Parlimen' order by CAST(kod AS unsigned) ASC";
	$query_mesyuarat = "select butiran from konfigurasi where kategori='Mesyuarat Parlimen' order by CAST(kod AS unsigned) ASC";
	$query_soalan = "select distinct(butiran) from konfigurasi where kategori='Bentuk Soalan' order by kategori ASC";
	$query_parti = "SELECT DISTINCT(butiran) FROM konfigurasi WHERE kategori='Parti' ORDER BY kategori ASC";	
	$query_bahagian = "select distinct(butiran) from konfigurasi where kategori='Tindakan' order by butiran ASC";
	$query_minggu = "SELECT butiran FROM konfigurasi WHERE kategori='Minggu Parlimen' ORDER BY CAST(kod AS unsigned) ASC";
	$query_negeri = "SELECT butiran FROM konfigurasi WHERE kategori='Negeri' ORDER BY butiran ASC";
	$query_minggu2 = "SELECT kod,butiran FROM konfigurasi WHERE kategori='Minggu Parlimen' ORDER BY CAST(kod AS unsigned) ASC";
	$query_jabatan = "SELECT butiran FROM konfigurasi WHERE kategori='Jabatan' ORDER BY butiran ASC";
	
	//$link_parlimen="https://".$_SERVER['HTTP_HOST']."/ssjp/"."parlimen/login.php?action=details&id="; //asal pada 20 feb
	$link_parlimen="http://".$_SERVER['HTTP_HOST']."/ssjp/parlimen/login.php?action=details&id=";
	//$link_parlimen="http://".$_SERVER['HTTP_HOST']."/ssjp/parlimen/login.php?action=details&id=";
	$link_bahas="http://".$_SERVER['HTTP_HOST']."/ssjp/"."parlimen/login.php?action=detailsbahas&id=";
	$link_kal = "http://".$_SERVER['HTTP_HOST']."/ssjp/"."parlimen/login.php?action=detailsKal&id=";
   


	$nama_sistem = "Sistem Pengesanan Soal Jawab Parlimen ";
	$sistem_kal = "Kalendar Parlimen";
	$teks_akhir = array("KSP","SUB(O)","KPSU(O)");
	
	function Reverse($date){ 
		if(!empty($date))
		{
			$result = (strpos($date,"-"));
	
			if($date == "0000-00-00")
				return $date="";
			if($result === false)
			{
				return $date;	
			}
			else
			{		
 	   			$temp = array();
 	   		 	$temp =  explode("-",$date); 
	   	 		$temp2 = array($temp[2], $temp[1], $temp[0]);
		 		$date = implode("/", $temp2);
	     		return $date;		
			}	
		}
	}

	function checkPengurusan($conn,$db,$userId){
		$qry = "SELECT Id FROM pengguna WHERE Id='$userId' AND Jenis=7";
		mysql_select_db($db,$conn) or die(mysql_error());
		$result = mysql_query($qry,$conn) or die(mysql_error());
		return (mysql_num_rows($result)>0)?1:0;			
	}
	function formatDate($mysql_date){
		if($mysql_date == "0000-00-00") return "";
		$dt = explode("-",$mysql_date);
		$english_date = date("j F Y",mktime(0,0,0,$dt[1],$dt[2],$dt[0]));	
		$month = array("January","February","March","April","May","June","July","August","September","October","November","December");
		$bulan = array("JANUARI","FEBRUARI","MAC","APRIL","MEI","JUN","JULAI","OGOS","SEPTEMBER","OKTOBER","NOVEMBER","DISEMBER");
		return str_replace($month,$bulan,$english_date);
	}
	function DisplayDate($dt){
		//return date to format dd/mm/yyyy
		$res = mysql_query("SELECT DATE_FORMAT('$dt','%d/%m/%Y') AS Tarikh");
		$row = mysql_fetch_array($res);
		return $row['Tarikh'];
	}
	function DisplayDateTime($dt){
		//return date to format dd/mm/yyyy hh:mm:ss
		$res = mysql_query("SELECT DATE_FORMAT('$dt','%d/%m/%Y %h:%i:%s %p') AS Tarikh");
		$row = mysql_fetch_array($res);
		return $row['Tarikh'];
	}
	function MysqlDate($dt){
		if($dt=='') return '';
		$td = explode("/",$dt);
		return date("Y-m-d", mktime(0,0,0,$td[1],$td[0],$td[2]));
	}
	function MysqlDate2($dt){
		if($dt=='') return '';
		$td = explode("/",$dt); 
		$localtime = localtime();
		return date("Y-m-d h:i:s", mktime($localtime[2],$localtime[1],$localtime[0],$td[1],$td[0],$td[2]));
	} 
	function PrintOption($conn,$db='$db_voffice', $query, $def=""){		
		mysql_select_db ($db, $conn) or die (mysql_error());		
		$result = mysql_query($query, $conn)or die(mysql_error());
		while ($rows = mysql_fetch_array($result)) { 
			if($def<>$rows['butiran'])
				echo "<option>".$rows['butiran']."</option>";
			else
				echo "<option selected>".$rows['butiran']."</option>";			
		}
	}
	
	function PrintOption2($conn,$db='$db_voffice', $query, $def=""){		
		mysql_select_db ($db, $conn) or die (mysql_error());		
		$result = mysql_query($query, $conn)or die(mysql_error());
		while ($rows = mysql_fetch_array($result)) { 
			if($def<>$rows['butiran'])
				echo '<option value="'.$rows['kod'].'">'.$rows['butiran']."</option>";
			else
				echo '<option value="'.$rows['kod'].'" selected>'.$rows['butiran']."</option>";	
		}
	}
	function PrintOption3($conn,$db='$db_voffice', $query, $def=""){		
		mysql_select_db ($db, $conn) or die (mysql_error());		
		$result = mysql_query($query, $conn)or die(mysql_error());
		while ($rows = mysql_fetch_array($result)) { 
			if($def<>$rows['kod'])
				echo '<option value="'.$rows['kod'].'">'.$rows['butiran']."</option>";
			else
				echo '<option value="'.$rows['kod'].'" selected>'.$rows['butiran']."</option>";	
		}
	}		
	function isAdmin($fullname,$admin){
		foreach($admin as $nama){
			if($nama==$fullname) return true;
		}
		return false;		
	}
	
	function sendMail($conn,$db,$fullname,$subject,$url){		
		mysql_select_db($db,$conn) or die(mysql_error());		
		$result = mysql_query("SELECT Emel FROM pengguna where Nama='$fullname'") or die(mysql_error());
			
		if(mysql_num_rows($result)==0){
			return false;
		}else{			
			$row = mysql_fetch_array($result);
			$sendTo = $row['Emel'];			
			$message = "Tuan/Puan\n\nSila klik URL di bawah untuk keterangan lanjut\n\n$url\n\nSekian, terima kasih. ";
			$headers = "From: \n";
			if(mail($sendTo, $subject, $message,$headers)){				
				return true;
			}else
				return false;
		}		
	}
	
	function getJawatan($conn,$jawatan){
	//function getbhg($conn,$jawatan){

		$qry = "SELECT emel,agensi_id FROM pengguna where jawatan='$jawatan'";
		$result = mysql_query($qry,$conn) or die(mysql_error());


		$i=0;
		while($row = mysql_fetch_array($result))
		{
			if(!empty($row['emel']))
			{
				$address.= $sap.$row['emel'];
				$sap = ", "; //echo $row['emel'];
			}
		}
		return $address;
	}
	
	function getAP($conn,$jawatan){
		$qry = "SELECT butiran FROM konfigurasi WHERE kategori = '$jawatan'";
		$result= mysql_query($qry,$conn) or die(mysql_error());
		
		while($row = mysql_fetch_array($result))
		{
			$jwtn = $row['butiran'];
			$qry2 = "SELECT emel FROM pengguna where jawatan='$jwtn'";
			$result2 = mysql_query($qry2,$conn) or die(mysql_error());
	
			if(mysql_num_rows($result2)>0)
			{
				$row2 = mysql_fetch_array($result2);
				$emel = $row2['emel'];
				if(!empty($emel))
				{
					$address.= $sap.$emel;
					$sap = ","; //echo $row['emel'];
				}
			}			
		}
		return $address;
	}
	
	//function sendTopttksusk($conn,$subject,$message)
	//{
	
	
	//}
	
	function sendSalinan($conn,$salinan,$subject,$message){		
	
		foreach ($salinan as $jawatan){
		
			if($emel = getJawatan($conn,$jawatan)){ 
			//if($emel = getbhg($conn,$jawatan)){ 
			$address.= $sap.$emel;
			//$address_.= $sap.$emel;
			$sap = ",";
			}
		}
	
		$from = $_SESSION['emel'];		 
		$headers = "From: ".$from."\n";		
		if(mail($address,$subject,$message,$headers)){			
		//if(mail($address_ ,$subject,$message,$headers)){			
			return $address;
			//return $address_;
		}else
			return false;		
	}
	
	//jamlee - edited
	
	function sendSalinanKepada($conn,$salinan,$subject,$message){		

		$sap ="";
		foreach ($salinan as $jawatan){
/*
		$qry_agensi_id = "SELECT agensi_id FROM pengguna WHERE jawatan='$jawatan'";
		$result_agensi_id = mysql_query($qry_agensi_id,$conn) or die(mysql_error());
		$row_agensi_id = mysql_fetch_array($result_agensi_id);

		$qry_sub = "SELECT emel FROM pengguna WHERE agensi_id='".$row_agensi_id['agensi_id']."' and jawatan LIKE '%sub%'";
		$result_sub = mysql_query($qry_sub,$conn) or die(mysql_error());
		$row_sub = mysql_fetch_array($result_sub);
		
		echo "<br>".$row_agensi_id['agensi_id']." ".$row_sub['emel']."<br>".$jawatan;
*/

		
			if($jawatan=="Anggota Pentadbiran")
			{
				if($emel = getAP($conn,$jawatan)){ 
					$addressAP.= $emel;
				}
			}else{			
				if($emel = getJawatan($conn,$jawatan)){ 
					$addressSK.= $sap.$emel;
					$sap = ", ";
				}
			}
		}
				
		$from = $_SESSION['emel'];		
		$headers = "From: ".$from."\n";	
		$error=false;
		
		if(!empty($addressAP)){
			if(mail($addressAP,$subject,$message,$headers))
				$error=true;
			else
				echo "Email tidak dihantar kepada (anggota pentadbiran) : ".$addressAP."<br>";
		}

		if(!empty($addressSK)){
			if(mail($addressSK,$subject,$message,$headers))
				$error=true;
			else
				echo "Email tidak dihantar kepada : ".$addressSK."<br>";
		}
		if(!empty($addressSK) && !empty($addressAP))
			$address = $addressAP.", <br />".$addressSK;
		elseif(!empty($addressSK) && empty($addressAP))
			$address = $addressSK;
		if(empty($addressSK) && !empty($addressAP))
			$address = $addressAP; 
		
		if($error){
			return $address;
		}else
			return false;
	}

	//jamlee - edited

	function getEmail($conn,$id){
		$result = mysql_query("SELECT emel FROM pengguna WHERE agensi_id='$id' AND roles LIKE '%9%'",$conn) or die(mysql_error());
		//echo "SELECT emel FROM pengguna WHERE agensi_id='$id' AND roles LIKE '%9%'";  
		$i=0;
		while($row = mysql_fetch_array($result))
		{
			if(!empty($row['emel']))
			{
				$address.= $sap.$row['emel'];
				$row['emel'].'<br>';
				$sap = ",";
			}
		}
		return $address;
	}
	
	function sendToMKll($conn,$jaw,$subject,$message){ 
		
		//echo "nn".$agensi2;
		// $salinan3=array();
	 $resultp="SELECT emel FROM pengguna WHERE jawatan='$jaw'";
	  $result2 =mysql_query($resultp,$conn);
	 
	//$result = mysql_query("SELECT emel FROM pengguna WHERE roles LIKE '%3%'AND $modul=1 ",$conn) or die(mysql_error());
		//$i=0;
		while($row = mysql_fetch_array($result2))
		{
			if(!empty($row['emel']))
			{
				$address.= $sap.$row['emel'];		
				$sap = ",";
			}
		}

		if(empty($address)){
			return false;
		}else{			
		$from = $_SESSION['emel'];		
		$headers = "From: ".$from."\n";	
		if(mail($address, $subject, $message,$headers)){			
			return $address;
		}else
			return false;
		}
	}
	
	
		function sendSalinanpatksp($conn,$emailpa,$subject,$message){		
	
		foreach ($emailpa as $jawatan){
		
			if($emel = getJawatan($conn,$jawatan)){ 
			//if($emel = getbhg($conn,$jawatan)){ 
			$address.= $sap.$emel;
			//$address_.= $sap.$emel;
			$sap = ",";
			}
		}
	
		$from = $_SESSION['emel'];		 
		$headers = "From: ".$from."\n";		
		if(mail($address,$subject,$message,$headers)){			
		//if(mail($address_ ,$subject,$message,$headers)){			
			return $address;
			//return $address_;
		}else
			return false;		
	}
	
	
	
	
	
	
	
	
	
function sendToKSP($conn,$jaw,$subject,$message){
		
		//echo "nn".$agensi2;
		// $salinan3=array();
	 $resultp="SELECT emel FROM pengguna WHERE jawatan='$jaw'";
	  $result2 =mysql_query($resultp,$conn);
	 
	//$result = mysql_query("SELECT emel FROM pengguna WHERE roles LIKE '%3%'AND $modul=1 ",$conn) or die(mysql_error());
		//$i=0;
		while($row = mysql_fetch_array($result2))
		{
			if(!empty($row['emel']))
			{
				$address.= $sap.$row['emel'];		
				$sap = ",";
			}
		}

		if(empty($address)){
			return false;
		}else{			
		$from = $_SESSION['emel'];		
		$headers = "From: ".$from."\n";	
		if(mail($address, $subject, $message,$headers)){			
			return $address;
		}else
			return false;
		}
	}
	
	
	
	
	
	
	
	
	
	
	
function sendToPPPB($conn,$agensi2,$subject,$message){
		
		//echo "nn".$agensi2;
		// $salinan3=array();
	 $resultp="SELECT emel FROM pengguna WHERE agensi_id='$agensi2' AND roles LIKE '%9%'";
	  $result2 =mysql_query($resultp,$conn);
	 
	//$result = mysql_query("SELECT emel FROM pengguna WHERE roles LIKE '%3%'AND $modul=1 ",$conn) or die(mysql_error());
		//$i=0;
		while($row = mysql_fetch_array($result2))
		{
			if(!empty($row['emel']))
			{
				$address.= $sap.$row['emel'];		
				$sap = ",";
			}
		}

		if(empty($address)){
			return false;
		}else{			
		$from = $_SESSION['emel'];		
		$headers = "From: ".$from."\n";	
		if(mail($address, $subject, $message,$headers)){			
			return $address;
		}else
			return false;
		}
	}
	
	
	
	
	
	
		function sendToPegawai2($conn,$agensi1,$subject,$message){
		
		//echo "nn".$agensi;
		 $salinan2=array();
	 $qryemel="select * from pengguna where agensi_id='$agensi1'";
	 $resultemel =mysql_query($qryemel,$conn);
	 // $i=0;
	 while($rowemel = mysql_fetch_array($resultemel)){
	    //$salinan=implode("+",$rowemel['jawatan']);
		 $salinan2[]=$rowemel['id']; 
		 //echo "ya".$rowemel['id'];
		//$i=$i+1;
		}
		// $salinan2[]=56;
		foreach ($salinan2 as $node){
		//echo "iyo";
		//$qry_sub = "SELECT emel FROM pengguna WHERE agensi_id='$node' and jawatan LIKE '%sub%'";
		$qry_sub = "SELECT emel FROM pengguna WHERE id='$node'"; 
		$result_sub = mysql_query($qry_sub,$conn) or die(mysql_error());
		$row_sub = mysql_fetch_array($result_sub);
		
		//echo "<br>".$node." ".$row_sub['emel']."<br>".$jawatan;
         // $find="takjumpa";
		 // echo "iyo";
		if(!empty($row_sub['emel']))
		{
			$address_.= $saps.$row_sub['emel'];			
			$saps = ",";
			//$find="jumpa";
			
		}
			
			//echo "nn".$find;
			//echo 'NODE='.$node.'<br>'; //rizal
			//if($emel = getEmail($conn,$node))
			//{ 
				//node=agensi_id
				//$address_.= $sap.$emel;
				//$sap = ",";
			//}
		   }
		   
		//$address_ = $address_.",".$row_sub['emel'];
		//$address_ = $address_.",".$row_sub['emel'];
        //$ggg="cccc";
		//echo $address_.'<br>';  //RIZAL
		$from = $_SESSION['emel'];		
		$headers = "From: ".$from."\n";	
		if(mail($address_,$subject,$message,$headers)){			
			return   $address_;
		}else
			return false;
	}


	function sendToPegawai3($conn,$tempdraf,$subject,$message)
	{
	foreach ($tempdraf as $node){
		//echo "iyo";
		//$qry_sub = "SELECT emel FROM pengguna WHERE agensi_id='$node' and jawatan LIKE '%sub%'";
		$qry_sub = "SELECT emel FROM pengguna WHERE jawatan='$node'"; 
		$result_sub = mysql_query($qry_sub,$conn) or die(mysql_error());
		$row_sub = mysql_fetch_array($result_sub);
		
		//echo "<br>".$node." ".$row_sub['emel']."<br>".$jawatan;
         // $find="takjumpa";
		 // echo "iyo";
		if(!empty($row_sub['emel']))
		{
			$address_.= $saps.$row_sub['emel'];			
			$saps = ",";
			//$find="jumpa";
			
		}
			
			//echo "nn".$find;
			//echo 'NODE='.$node.'<br>'; //rizal
			//if($emel = getEmail($conn,$node))
			//{ 
				//node=agensi_id
				//$address_.= $sap.$emel;
				//$sap = ",";
			//}
		   }
		   
		//$address_ = $address_.",".$row_sub['emel'];
		//$address_ = $address_.",".$row_sub['emel'];
        //$ggg="cccc";
		//echo $address_.'<br>';  //RIZAL
		$from = $_SESSION['emel'];		
		$headers = "From: ".$from."\n";	
		if(mail($address_,$subject,$message,$headers)){			
			return   $address_;
		}else
			return false;
	
	}
	
	
	//jamlee - edited
	
	function sendToPegawai($conn,$agensi,$subject,$message){
		
		
		foreach ($agensi as $node){
		
		//$qry_sub = "SELECT emel FROM pengguna WHERE agensi_id='$node' and jawatan LIKE '%sub%'";
		$qry_sub = "SELECT emel FROM pengguna WHERE agensi_id='$node' and roles LIKE '%6%'"; 
		$result_sub = mysql_query($qry_sub,$conn) or die(mysql_error());
		$row_sub = mysql_fetch_array($result_sub);
		//echo $qry_sub;
		//echo "<br>".$node." ".$row_sub['emel']."<br>".$jawatan;

		if(!empty($row_sub['emel']))
		{
			$address_sub.= $saps.$row_sub['emel'];			
			$saps_sub = ",";
		}
			
			
			//echo 'NODE='.$node.'<br>'; //rizal
			if($emel = getEmail($conn,$node))
			{ 
				//node=agensi_id
				//echo "mail".$emel.'<br>';
				//echo "mail".$emel."<br>";
				$address_.= $sap.$emel;
				$sap = ",";
			}
		   }
		   
		$address_ = $address_.",".$row_sub['emel'];
//echo "bbb".$address_ ;
		//echo $address.'<br>';  //RIZAL
		//echo"xxx". $address.'<br>';  //RIZAL
		
		$from = $_SESSION['emel'];		
		$headers = "From: ".$from."\n";	
		if(mail($address_,$subject,$message,$headers)){			
			return $address_;
		}else
			return false;
			
	}

	//jamlee - edited
	
	function cariEmel($conn,$db,$fullname){
		mysql_select_db($db,$conn) or die(mysql_error());
		$result = mysql_query("SELECT Emel FROM pengguna WHERE Nama='$fullname'", $conn);
		$row = mysql_fetch_array($result);
		return $row['Emel'];
	}
	
	function sendAtasan($conn,$parlimen_id,$subject,$message){
		$qry = "SELECT penyemak FROM parlimen WHERE id='$parlimen_id'";
		$result = mysql_query($qry,$conn) or die(mysql_error());
		$row=mysql_fetch_array($result);
		$penyemak = explode("+",$row['penyemak']);

		foreach ($penyemak as $jawatan)
		{
			if($emel = getJawatan($conn,$jawatan)){ //node=agensi_id
				$address.= $sap.$emel;		
				$sap = ", ";
			}
		}
				
		if(empty($address)){
			return false;
		}else{			
		$from = $_SESSION['emel'];		
		$headers = "From: ".$from."\n";	
		if(mail($address, $subject, $message,$headers)){			
			return $address;
		}else
			return false;
		}
	}
	
	function sendAtasanBahas($conn,$bahas_id,$subject,$message){
		$qry = "SELECT penyemak FROM sesi_bahas WHERE id='$bahas_id'";
		$result = mysql_query($qry,$conn) or die(mysql_error());
		$row=mysql_fetch_array($result);
		$penyemak = explode("+",$row['penyemak']);

		foreach ($penyemak as $jawatan)
		{
			if($emel = getJawatan($conn,$jawatan)){ //node=agensi_id
				$address.= $sap.$emel;		
				$sap = ", ";
			}
		}
				
		if(empty($address)){
			return false;
		}else{			
		$from = $_SESSION['emel'];		
		$headers = "From: ".$from."\n";	
		if(mail($address, $subject, $message,$headers)){			
			return $address;
		}else
			return false;
		}
	}
 
	function sendHEK($conn,$modul,$subject,$message){
		
		$result = mysql_query("SELECT emel FROM pengguna WHERE roles LIKE '%3%'AND $modul=1 ",$conn) or die(mysql_error());
		$i=0;
		while($row = mysql_fetch_array($result))
		{
			if(!empty($row['emel']))
			{
				$address.= $sap.$row['emel'];		
				$sap = ",";
			}
		}

		if(empty($address)){
			return false;
		}else{			
		$from = $_SESSION['emel'];		
		$headers = "From: ".$from."\n";	
		if(mail($address, $subject, $message,$headers)){			
			return $address;
		}else
			return false;
		}
	}
	
	function getTeksAkhir($conn,$cat){
		$teks_akhir = array();
		$qry = "SELECT butiran FROM konfigurasi WHERE kategori = '$cat'";
		$result= mysql_query($qry,$conn) or die(mysql_error());
		while($row = mysql_fetch_array($result))
		{
			$teks_akhir[] = $row['butiran'];
		}
		return $teks_akhir;
	}
	
	function sendTeksAkhir($conn,$cat,$subject,$message){
		
		$teks_akhir = getTeksAkhir($conn,$cat);
		foreach($teks_akhir as $jawatan)
		{
		   //echo $jawatan;
			$qry = "SELECT emel,agensi_id FROM pengguna WHERE jawatan='$jawatan'";
			$result = mysql_query($qry,$conn) or die(mysql_error());
			$row = mysql_fetch_array($result);

			$qry_agensi_id = "SELECT agensi_id FROM pengguna WHERE nama='".$_SESSION['nama']."' and jawatan='".$_SESSION['jawatan']."' LIMIT 1";
			$result_agensi_id = mysql_query($qry_agensi_id,$conn) or die(mysql_error());
			$row_agensi_id = mysql_fetch_array($result_agensi_id);
			//echo $row_agensi_id['agensi_id'];

			$qry_sub = "SELECT emel FROM pengguna WHERE agensi_id='".$row_agensi_id['agensi_id']."' and jawatan LIKE '%sub%' LIMIT 1";
			$result_sub = mysql_query($qry_sub,$conn) or die(mysql_error());
			$row_sub = mysql_fetch_array($result_sub);
			//echo $jawatan.$row_sub['emel']."<BR>";
			
			if(!empty($row_sub['emel']))
			{
				$address_sub.= $saps.$row_sub['emel'];			
				$saps_sub = ",";
			}

			
			if(!empty($row['emel']))
			{
				$address_.= $saps.$row['emel'];			
				$saps = ",";
			}
		}	
		
		//$address_ = $address_.",".$row_sub['emel'].$_POST['sub_email'];
		//$address_ = $row_sub['emel'].$_POST['sub_email']; asal
		$address_ = $address_.",".$row_sub['emel'];
		
		if(empty($address_)){
			return false;
		}else{			
			$from = $_SESSION['emel'];		
			$headers = "From: ".$from."\n";	
			if(mail($address_, $subject, $message,$headers)){			
				return $address_;
			}else
				return false;
		}
	}
	
	function sendDPA($conn,$db,$subject,$url){
		mysql_select_db($db,$conn) or die(mysql_error());
		$result = mysql_query("SELECT Emel FROM pengguna WHERE Jenis=6",$conn) or die(mysql_error());		
		while($row=mysql_fetch_array($result)){
			$address.= $sap.$row['Emel'];			
			$sap = ",";
		}
		$message = "Tuan/Puan\n\nSila klik URL di bawah untuk keterangan lanjut\n\n$url\n\nSekian, terima kasih. ";
		if($address<>""){
			$headers = "From: \n";
			if(mail($address,$subject,$message,$headers)) return true;					
		}
		return false;
	}	
	

	function checkDPA($conn,$db,$userId){
		$qry = "SELECT Id FROM pengguna WHERE Id='$userId' AND Jenis=6";
		mysql_select_db($db,$conn) or die(mysql_error());
		$result = mysql_query($qry,$conn) or die(mysql_error());
		return (mysql_num_rows($result)>0)?1:0;			
	}
	function checkAdmin($conn,$db,$userId){
		$qry = "SELECT Id FROM pengguna WHERE Id='$userId' AND Kategori='admin'";
		mysql_select_db($db,$conn) or die(mysql_error());
		$result = mysql_query($qry,$conn) or die(mysql_error());
		return (mysql_num_rows($result)>0)?1:0;	
	}
	function splitNama($string){
		$name = array();
		$len = strlen($string);
		$pos1 = strpos($string,',');
		$pos2 = strpos($string,' (');
		
		$pos = ($pos1 < $pos2)?$pos1:$pos2;
		$sap2 = ($pos1 < $pos2)?"(":",";			
		
		$name[0] = trim(substr($string,0,$pos));
		
		$pos++;
		
		if($name[0]==""){
			$pos = strpos($string,$sap2);
			$name[0] = trim(substr($string,0,$pos));
		}
		if($name[0]=="") $name[0] = $string;
		if($pos<>"") $name[1] = trim(substr($string,$pos,$len));
		return $name;
	}
	function ExtractDept($data){
		$dept = explode(";",$data);
		foreach($dept as $node){
			$jab1 = explode("+",$node);
			$jabatan[] = $jab1[0];
		}
		return implode(",",$jabatan);
	}
	
	function checkView($conn,$field,$key,$userid){
		$qry = "SELECT $field FROM pengguna WHERE Id='$userid'";
		$result = mysql_query($qry,$conn) or die(mysql_error());
		$row = mysql_fetch_array($result);
		//$items = $row[$field]<>""?explode(";",$row[$field]):(return false);
		if($row[$field]<>"")
			$items = explode(";",$row[$field]);
		else
			return false;
		if(in_array($key,$items)){
			return true;
		}
		else {
			return false;
		}
	}
	function SelectAgensi($conn,$agen){
		$agensi = explode(";",$agen);
		$qry = "SELECT butiran FROM konfigurasi WHERE kategori='Jabatan' ORDER BY butiran";
		$result = mysql_query($qry,$conn) or die(mysql_error());
		while($row =mysql_fetch_array($result)){
			$value = $row['butiran'];	
			$checked = in_array($value,$agensi)?"checked":"";
			$msg .= "<div class=\"j\"><div class=\"j1\"><input onClick=\"agensiUtama(this)\" id=\"agensi\" type=\"checkbox\" $checked name=\"Agensi[]\"   value=\"$value\"></div><div class=\"j2\">$value</div></div>";
		}
		
		return $msg;
		
	}
	
	function getExpired($conn,$id){
		$qry = "SELECT ADDDATE(Tarikh,INTERVAL 30 DAY) AS NewDate from memorandum 
		where PId='$id' order by NewDate DESC limit 1";
		$res = mysql_query($qry,$conn);
		if(mysql_num_rows($res)>0){
			$row = mysql_fetch_array($res);
			$dt = explode("-",$row['NewDate']);
			$_date = mktime(0,0,0,$dt[1],$dt[2],$dt[0]);
			
			if($_date<time())
				return "<img src=\"../images/senyum.gif\"/>";
			else
				return "";
		}
		else
			return "";
	}
	
	function isExpired($mysql_date,$status){		
		if($status == "" || $status==null) return "";
		$dt = explode("-",$mysql_date);
		$_date = mktime(0,0,0,$dt[1],$dt[2],$dt[0]);
		if($_date<time() && $status <> '12'){				
			return "<img src=\"../images/senyum.gif\"/>";
		}
		else
			return "";
	}
	
	function getTindakanTerakhir($id,$conn){
		$qry = "SELECT Tindakan FROM memorandum WHERE Id='$id' LIMIT 1";
		$result = mysql_query($qry,$conn) or die(mysql_error());
		$row = mysql_fetch_array($result);
		return $row['Tindakan'];
	}
	function menuClick($menuClick,$menu){
		if($menuClick == $menu)
			return "color:#CD3700";	
	}
	
	function selectKawasan($conn,$db,$def="",$sesi_dewan="Dewan Rakyat"){
	mysql_select_db($db,$conn) or die(mysql_error());
	$qry = "SELECT kawasan_id FROM ahli_parlimen 
	WHERE kawasan_id <>'' AND sesi_dewan='$sesi_dewan' ORDER BY kawasan_id ASC";
	$result = mysql_query($qry) or die(mysql_error());
	
	while($rows = mysql_fetch_array($result)){
		$id = $rows['kawasan_id'];
		$qry2 = "SELECT nama FROM kawasan WHERE id ='$id'";
		$res = mysql_query($qry2) or die(mysql_error());
		$row2 = mysql_fetch_array($res);
		if($def<>$row2['nama'])
			echo "<option value =\"$id\">".$row2['nama']."</option>";
		else
			echo "<option selected>".$row2['nama']."</option>";
	}
}
	
	function createRTF($sBasePath, $fieldName, $defaultValue){
		//jamlee edit
		// Rich Text Field
		// Automatically calculates the editor base path based on the _samples directory.
		// This is usefull only for these samples. A real application should use something like this:
		// $oFCKeditor->BasePath = '/fckeditor/' ;	// '/fckeditor/' is the default value.
				
	//	$oFCKeditor = new FCKeditor('FCKeditor1') ;
		$oFCKeditor = new FCKeditor($fieldName) ;
		$oFCKeditor->BasePath = $sBasePath ;
		$oFCKeditor->Config['ToolbarStartExpanded'] = true ;
		$oFCKeditor->Height = '200';
		
		if ( isset($_GET['Skin']) )
			$oFCKeditor->Config['SkinPath'] = $sBasePath . 'editor/skins/' . htmlspecialchars($_GET['Skin']) . '/' ;
		
		$oFCKeditor->Value = $defaultValue ;
		$oFCKeditor->ToolbarSet	= 'SpmjmSpsp' ;
		$oFCKeditor->Create();	
	}	
	
	function getInfo($table, $id, $field)
	{
		$qry = "SELECT $field FROM $table WHERE id ='$id'";
		$result = mysql_query($qry) or die(mysql_error());
		$row = mysql_fetch_array($result);
		return $row[$field];		
	}
	
	function getSemakanParlimen($conn, $cat)		
	{
		$q = "SELECT kod, butiran FROM konfigurasi WHERE kategori='$cat'";	
		$r = mysql_query($q,$conn);
		while($rows = mysql_fetch_array($r))
		{
			$kod = $rows['kod'];
			$butiran = $rows['butiran'];
			$td1 = "<span style=\"width=200px\"><input $checked type=\"checkbox\" name=\"salinan[]\" value=\"$butiran\" >
			<label id=\"kod\">".$kod."</label></span><br>"; 
			$td2 = "<span style=\"width=200px\"><input type=\"textfield\" name=\"catatan_semak[]\" id=\"catatan_semak[]\" value=\"Untuk Makluman\" disabled></span><br>"; 
			echo $td1;
			//echo $td2;
			
		}
	}	
	
	function getSemakanParlimen2($conn, $cat)		
	{
		$q = "SELECT kod, butiran FROM konfigurasi WHERE kategori='$cat'";	
		$r = mysql_query($q,$conn);
		while($rows = mysql_fetch_array($r))
		{
			$kod = $rows['kod'];
			$butiran = $rows['butiran']; 
			$td1 = "<span style=\"width=200px\"><input $checked type=\"checkbox\" name=\"semaktksp[]\" value=\"$butiran\" >
			<label id=\"kod\">".$kod."</label></span><br>"; 
			$td2 = "<span style=\"width=200px\"><input type=\"textfield\" name=\"catatan_semak[]\" id=\"catatan_semak[]\" value=\"Untuk Makluman\" disabled></span><br>"; 
			echo $td1;
			//echo $td2;
			
		}
	}	
	
	
	function lookup($conn, $tableLookup, $columnLookup, $condition){
		if (!empty($condition)){
			$sql = "SELECT $columnLookup FROM $tableLookup WHERE $condition";
			$rsLookup	= mysql_query($sql) or die(mysql_error());
			$numRows	= mysql_num_rows($rsLookup);
			
			$lookupValue	= "";
			while($rowLookup	= mysql_fetch_array($rsLookup, MYSQL_ASSOC)){
				$lookupValue[]	= $rowLookup[$columnLookup];
			}
			
			$lookupValue	= implode(", ",$lookupValue);
			return $lookupValue;
		}
		else
			return "";
	}	
	
	function findemel($conn,$idbhg)
	{
	 $qry = "SELECT distinct emel from pengguna where agensi_id ='$idbhg'";
		$result = mysql_query($qry,$conn) or die(mysql_error());
		
		$row = mysql_fetch_array($result);
		if(!empty($row['emel']))
		{
		$foundemel=$row['emel'];
		}
		
		return $foundemel;
	}
	function createRTF2($sBasePath, $fieldName, $defaultValue){
		
		$oFCKeditor = new FCKeditor($fieldName) ;
		$oFCKeditor->BasePath = $sBasePath ;
		$oFCKeditor->Config['ToolbarStartExpanded'] = true ;
		$oFCKeditor->Height = '200';
		
		if ( isset($_GET['Skin']) )
			$oFCKeditor->Config['SkinPath'] = $sBasePath . 'editor/skins/' . htmlspecialchars($_GET['Skin']) . '/' ;
		
		$oFCKeditor->Value = $defaultValue ;
		$oFCKeditor->ToolbarSet	= 'drafssjp' ;
		$oFCKeditor->Create();	
	}	
	function get_bentukSoalan($conn, $cat,$parlimen_id)		
	{
		$q = "SELECT kod, butiran FROM konfigurasi WHERE kategori='$cat'";	
		$r = mysql_query($q,$conn);
		$q2 = "SELECT jns_soalan FROM parlimen WHERE id='$parlimen_id'";	
		$r2 = mysql_query($q2,$conn);
		$rows2 = mysql_fetch_array($r2);
		//echo $rows2['jns_soalan'];
		while($rows = mysql_fetch_array($r))
		{
			//echo $rows2['jns_soalan'];
			//echo $rows['kod'];
			if($rows2['jns_soalan']==$rows['kod'])
			{
			$check="checked";
			}
			else
			{
			$check="unchecked";
			}
			//echo $check;
			$kod = $rows['kod'];
			$butiran = $rows['butiran']; 
			
			
			
			$td1 = "<span style=\"width=200px\"><input type=\"radio\" name=\"bentuk_soalan\" value=\"$kod\" $check  onClick=\"penerangan(this.value)\"/>
			<label id=\"kod\">".$butiran."</label></span><br>"; 
			echo $td1;
			//echo $td2;
			
		}
	}	
	
	function getcarifail($conn)
	  {
	 $qry = "SELECT nama_fail  from parlimen_lampiran  
		      ORDER BY nama_fail";
		
		$result = mysql_query($qry,$conn) or die(mysql_error());
		
		while($row = mysql_fetch_array($result)){
			//$id = $row['id'];
			$fail = $row['nama_fail']; 
			$fail_2 = $row['nama_fail']; 
			$fail=md5(base64_encode($fail)) 
			?>

			<?php
			$td = "<input  type=\"hidden\" name=\"Fail[]\" value=\"$fail\">"; 
			echo $td; 
		}
		
		}
?>