...
$qry = "SELECT pengguna.nama as nama,pengguna.agensi_id, pengguna.jawatan, pengguna.roles, agensi.nama AS agensi FROM pengguna,agensi WHERE pengguna.id = '$userid' AND pengguna.password=MD5('$password')";
...
//	$myAccess = new Access($ldapServer,$ldapPort,$ldapBase);	
	...
	//echo $action.$_POST['id'];
		...
		if(!empty($_POST['action'])){
			if ($_POST['action']=='details') {
					header("location:daftar/index.php?action=details&id=".$_POST['id']);
			}
			//}
		}else {		
			header("location:mainmenu.php");	
