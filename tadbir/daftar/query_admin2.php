<?php
	...
	if(!empty($app) && ($_GET['action']=="newdoc3")){
		echo $acl_denied;
		echo "</tr></table>";
		include("../footer.php");
		exit(0);
	...
	*/
	...
	function getAgensi($def,$conn,$kodjns){		 
	   if($kodjns==2)
	   {
	   $qry = "SELECT agensi.id AS id, agensi.nama, agensi.nama AS nama FROM agensi  where agensi.id in('54','53','50') ORDER BY nama_pendek";
	   }
	   else if($kodjns==1)
	   {
		$qry = "SELECT agensi.id AS id, agensi.nama, agensi.nama AS nama FROM agensi where agensi.id not in('54','53','50') ORDER BY nama_pendek";
        }
		else if($kodjns==3)
	   {
	...
	$userId = $_GET['id'];
	$qry = mysql_query("SELECT * FROM pengguna WHERE id = '$userId'");
	$row = mysql_fetch_array($qry);
	$sistem = $row['sistem'];
	
	$sql_role = mysql_query("select id, role from roles order by role");
...
var kp = form.Idkp.value;
		...
		if(jawatan < 1){
			msj = 'Sila masukkan Jawatan';
			alert(msj)
			form.Jawatan.focus();
			return false; 
		}
		if(emel.length < 1){
			msj = 'Sila masukkan group emel ssjp';
			alert(msj)
			form.Emel.focus();
			return false; 
		}
	...
	function validForm(form){ 
		...
		if(nokp.length < 1){
			msj = 'Sila masukkan No Kp pengguna';
			form.Idkp.focus();
			error = 1;
		} else {
			if(nokp.length != 12){
		...
		if(emel.length < 1){
			msj = 'Sila masukkan emel pengguna';
			form.Emel.focus();
			error = 1;
		}
