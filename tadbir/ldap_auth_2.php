   ...
   else
   {
		 ...
		 //Temporary Login to myexchange server  --remove this line to login to satuid
		  ...
		  { //if connection to ldap is working->proceed to login
		   ...
		   $login_ldap = ldap_bind($ldapconn2,$user_,$password) or die("<script>window.alert('No KP dan Katalaluan tidak sepadan ');location.href='index.php';</script>");
		   ...
		   // register sessions & values
			 ...
			 $query_rs_pengguna = "SELECT * from pengguna WHERE trim(nokp)='".trim($ic_)."'";
			 ...
			 //ambil maklumat pengguna dari database dan simpan dalam sessions
					...
					$_SESSION['jawatan'] = $ic_;	
					$_SESSION['valid'] = true;			
				// forward ke	page mainmenu.php
				
				
				  $logintime = date("Y-m-d H:i:s");
		          $qry = "INSERT INTO history (UserId,nokp,modul_id,Login) VALUES ('$name_','$ic_','$modul_id','$logintime')";
			 ...
			 echo ("<script>window.alert('Pengguna Tiada Didalam Senarai');location.href='index.php';</script>"); 
		 ...
		 //notify user
		 //back to login page
		 echo ("<script>window.alert('noKP dan Katalaluan Tidak Sepadan');location.href='index.php';</script>");
